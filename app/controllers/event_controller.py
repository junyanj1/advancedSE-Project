...
ISO_8601 = r'^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9]) \
...
class EventController():
    ...
    def create_event(self, organizer_id, title, description, location_name,
        ...
        Create new event
        """
        if self.check_event_input(organizer_id, title, description,
    ...
    def check_event_id(self, event_id):
        ...
        validated = False
        try:
            validated = Schema(And(str, lambda s: bool(
                                   re.match(
                                        "^[A-Za-z0-9]*$",
                                        s)))).validate(event_id)
    ...
    def check_event_input(self, organizer_id, title, description,
        ...
        Validate event data through regex check
        """
        schema = Schema({
